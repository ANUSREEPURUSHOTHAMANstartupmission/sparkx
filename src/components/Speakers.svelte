<script>
  ...
  onMount(() => {
    fetch(`https://events.startupmission.in/api/event/sparkx_web/speakers?category=speaker`)
  ...
  //   window.scroll({
  //     left:0,
  //     top:findPos(document.getElementById("speakers"))+500, 
  //     behavior:'smooth'
  //   });
  // }
</script>
  ...
  <section class="overflow-hidden openTrans relative">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
      {#each speakers as { name, designation, organisation, photo, linkedin }}
        <div class="relative group rounded-lg overflow-hidden flex flex-col border-2 h-full shadow-sm hover:shadow-lg transition-all duration-300 border-white">
          <img src={getImage(photo)} alt={name} class="w-full object-cover p-8" loading="lazy" />

          <div class="text-black text-center py-2 px-2 font-semibold h-full flex-1 bg-white">
            <h3 class="text-base">{name}</h3>
              <p class="text-xs font-normal">{designation}</p>

              {#if organisation}
                <p class="text-xs font-normal">{organisation}</p>
              {/if}
          </div>
          ...
          {#if linkedin}
            <a href={linkedin} target="_blank"
               class="absolute bottom-0 left-8 transform -translate-x-1/2 translate-y-full 
                      ...
                      rounded-full p-2 z-10 border border-gray-300 text-gray-700">
